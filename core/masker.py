...
class PIIMasker:
    ...
    DEPRECATED: Use Masker class instead for better testability.
    ...
    def __init__(self, config: dict[str, Any] | None = None):
        ...
        self._masker = Masker(config=self.config)
    ...
    def mask(
        ...
        """
        result = self._masker.mask(text, language=language, do_preprocess=do_preprocess)
        entities_info = result.to_entities_info() if verbose else None
        return result.masked_text, entities_info
...
def mask_pii_in_text(
    ...
    result = masker.mask(text, language=language, do_preprocess=preprocess)
    entities_info = result.to_entities_info() if verbose else None
    return result.masked_text, entities_info
