...
try:
    ...
    from recognizers import GinzaAddressRecognizer, GinzaPersonRecognizer
...
def create_default_registry(
    ...
    if use_transformer and TRANSFORMER_AVAILABLE:
        # Use ModelRegistry for centralized model management
        from model_registry import ModelRegistry

        model_registry = ModelRegistry(app_config)

        # Register Transformer recognizers from ModelRegistry
        for model_id in model_registry.list_model_ids():
            model_config = model_registry.get_config(model_id)
            if model_config:
                recognizer = model_registry.get(model_id)
                if recognizer:
                    language = model_config.get("language", "en")
                    entities = model_config.get("entities", [])
                    model_name = model_config.get("model_name", "unknown")
                    description = model_config.get("description", f"NER via {model_name}")
